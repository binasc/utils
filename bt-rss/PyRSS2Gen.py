...
class handler_wrapper():
    ...
    def startElement(self, name, attrs):
        self.__handler.startElement(name, attrs)
        #self.__handler._out.write(u'<![CDATA[')
        self.__handler._write(u'<![CDATA[')

    def endElement(self, name):
        self.__handler._write(u']]>')
        self.__handler.endElement(name)

    def characters(self, content):
        #self.__handler.characters(content)
        self.__handler._write(unicode(content.replace(']]>', ']]>]]&gt;<![CDATA['), 'utf-8'))
        #self.__handler._out.write(content)
...
class RSSItem(WriteXmlMixin):
    ...
    def publish(self, handler):
        ...
        self.publish_extensions(handler)
        wrapper = handler_wrapper(handler)
        _opt_element(wrapper, "description", self.description)
