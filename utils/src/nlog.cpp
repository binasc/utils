...
bool NLog:: init(map<string,string> linfo)
{
	...
	if(linfo.end() != linfo.find("level")) set_max_level((LogLevel)atoi(linfo["level"].c_str()));
	if(linfo.end() != linfo.find("enable_usec")) set_usec((utils::LogLevel)(atoi(linfo["enable_usec"].c_str())));
	open();
	return true;
}
